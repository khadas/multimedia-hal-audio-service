...
static ::android::sp<AudioClientBinder> client;
static int inited = 0;
...
static char* stream_get_parameters(const struct audio_stream* stream, const char* keys)
{
...
{
    return client->Device_set_master_mute(mute);
}

static int Device_get_master_mute(struct audio_hw_device *dev, bool *mute)
{
    return client->Device_get_master_mute(*mute);
}
...
{
    return client->Device_create_audio_patch(num_sources, sources, num_sinks, sinks, handle);
}

static int Device_release_audio_patch(struct audio_hw_device *dev,
                               audio_patch_handle_t handle)
{
    return client->Device_release_audio_patch(handle);
}

static int Device_set_audio_port_config(struct audio_hw_device *dev,
                         const struct audio_port_config *config)
{
    return client->Device_set_audio_port_config(config);
}
...
int audio_hw_load_interface(audio_hw_device_t **dev)
{
    ...
    client = new AudioClientBinder();
    client->setspInstance(client);
...
int audio_effect_set_parameters(aml_audio_effect_type_e type, effect_param_t* param)
{
    ...
    if (type >= AML_EFFECT_MAX || type < 0) return -1;
    if (param == NULL || param->psize == 0 || param->vsize == 0) return -1;
